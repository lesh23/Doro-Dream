...
tra_exp = pd.read_csv('1차지역선정_관광소비지수.csv')
ind = tra_exp['지역명']

# 안전지수 변수 선언 후 칼럼, index 수정
safety_score = pd.read_csv('지역_연도별_안전지수_minmax2.csv')
safety_score = safety_score.set_index(ind)
safety_score = safety_score.iloc[:,1:5]

# 관광지수 계산을 위한 관광소비지수, 관광지지수, 관광객수 변수 선언
tra_exp = pd.read_csv('1차지역선정_관광소비지수.csv')
tra_spot = pd.read_csv('1차지역선정_관광지지수.csv') 
traveler = pd.read_csv('전처리_시도별관광객수정규화.csv')
...
# 환경지수 변수 선언 
air = pd.read_csv('pollution_final.csv')
park = pd.read_excel('area_score.xlsx')
greenhouse = pd.read_csv('mm.csv')
...
total_chart.to_csv('1차지역선정_최종합산점수.csv', index=True, encoding = 'utf-8')
